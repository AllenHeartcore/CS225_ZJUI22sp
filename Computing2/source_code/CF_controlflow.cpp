...
void checktime(){
    ...
    if (passedhdays - num_localqueues > 0 ){//if we passed halfday, clear local queues and upload to center
        ...
        for (i=0;i<localqueue1.size();i++){
            ...
            int treatment = newnode->person->treatment;
            Fheap[treatment]->insert_node(newnode); //treatment
            week_report.size+=1;
            mon_report.size+=1;
        }
        localqueue1.clear();
    ...
    if ((passedhdays - 2*num_appointments) >= 2){
        ...
        for (i=0;i<IDsheet.size();i++){
            if (IDsheet[i]->deadline[0]>0){
                ...
                Fheap[treatment]->modify_key(IDsheet[i]->id, 1);
                }
            }
        }
        for (i=0;i<3;i++){
            for (j=0;j<5;j++){
                //5 people out per heap per day
                centernode* node = Fheap[i]->extract_min();
                if (node == NULL){
                    break;   //no more person in F-heap
                }
                else {
                    for (k=0;k<3;k++){
                        int index = node->person->registration->location_id[k];
                        if (hos[index-1]->come_hospital(node) == 1){   //-1!!!!!!!!
    ...
    for (i = 0; i < inhospital.size(); i++){//clacuate waited time
        ...
        for (j=0;j<IDsheet.size();j++){
            if (IDsheet[j]-> id == inhospital[i]->person->id){
                //delete from btree first 
                int64_t secondarykey = IDsheet[j]->secKey;
                btree.deletion(secondarykey);
                order_block.delete_record(IDsheet[j]->id);
                bplus_tree.remove(IDsheet[j]->id);
...
void report::generate_report(char* filename){
    ...
    if (filename == "weekly_report.csv"){
        ...
        outFile << ",name,profession,age_group,medical_risk,waiting_time,treatment_type" << endl;
        outFile << "Treated" << endl;
        for(int i = 0; i < treated.size(); i++){
            outFile << i << "," << treated[i]->person->name << "," << treated[i]->person->profession << "," << treated[i]->person->age_group << "," << treated[i]->person->medical_risk << "," << treated[i]->person->waited_time << "," << treated[i]->person->treatment << endl;
        }
        outFile << "Appointment" << endl;
        for(int i = 0; i < appointment.size(); i++){
            outFile << i << "," << appointment[i]->person->name << "," << appointment[i]->person->profession << "," << appointment[i]->person->age_group << "," << appointment[i]->person->medical_risk << "," << appointment[i]->person->waited_time << "," << appointment[i]->person->treatment << endl;
        }
        outFile << "Not_treated" << endl;
        for(int i = 0; i < not_treated.size(); i++){
            outFile << i << "," << not_treated[i]->person->name << "," << not_treated[i]->person->profession << "," << not_treated[i]->person->age_group << "," << not_treated[i]->person->medical_risk << "," << not_treated[i]->person->waited_time << "," << not_treated[i]->person->treatment << endl;
        }
        outFile.close();   
    }
    else{
