...
void BPlusTree::query(int64_t upper_bound, int64_t lower_bound){
    int i;
    if (root == NULL) {
    ...
    do {
        for (i = 0; i < ptr->size; i++) {
            if (ptr->patients[i]->id >= upper_bound) {
                if (ptr->patients[i]->id <= lower_bound) {
                    cout << ptr->patients[i]->id << " " << ptr->patients[i]->name << " " << ptr->patients[i]->age << endl;
                }
                else return;
            }
        }
